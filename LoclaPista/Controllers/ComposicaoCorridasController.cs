...
namespace LoclaPista.Controllers
{
    public class ComposicaoCorridasController : Controller
    {
        ...
        public ActionResult Create(int carroid, int carroid2, int carroid3, int carroid4,int pagamentoid,int pistaid,string data,string hora,int id, ComposicaoCorrida composicaoCorrida)
        {
            try
            {
                var dateTime = DateTime.ParseExact(hora, "HH:mm", null, DateTimeStyles.None);
                var dta = DateTime.Parse(data);
                if (dta>DateTime.Now && dateTime.Hour<=18 && dateTime.Hour >= 8)
                {
                    DateTime combined = dta.Date.Add(dateTime.TimeOfDay);
                    HorarioPista ho = HorarioPistaDAO.ProcurarbyDataHoraPista(combined, pistaid);
                    if (ho==null)
                    {
                        ...
                        if (ModelState.IsValid)
                        {
                            if (teste1.c.placa != teste2.c.placa | teste1.c.placa != teste3.c.placa | teste1.c.placa != teste4.c.placa | teste2.c.placa != teste3.c.placa | teste2.c.placa != teste4.c.placa | teste3.c.placa != teste4.c.placa | teste1.p.Cpf != teste2.p.Cpf | teste1.p.Cpf != teste3.p.Cpf | teste1.p.Cpf != teste4.p.Cpf | teste2.p.Cpf != teste3.p.Cpf | teste2.p.Cpf != teste4.p.Cpf | teste3.p.Cpf != teste4.p.Cpf)
                            {
                                ...
                                horariopista.Hora_inicial = combined;
                                horariopista.Hora_Final = combined.AddHours(1);
